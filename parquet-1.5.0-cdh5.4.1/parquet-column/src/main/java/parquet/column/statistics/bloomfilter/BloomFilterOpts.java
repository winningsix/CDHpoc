 ...
 * under the License.
 */
package parquet.column.statistics.bloomfilter;
...
public class BloomFilterOpts {
  public static class BloomFilterEntry {
     ...
     */
    public BloomFilterEntry(int numBits, int numHashFunctions) {
  ...
  Map<ColumnDescriptor, BloomFilterEntry> filterEntryList = new HashMap<>();
  ...
  public BloomFilterEntry getBloomFilterEntryByColName(ColumnDescriptor colName) {
    if (filterEntryList.containsKey(colName)) {
      return filterEntryList.get(colName);
    } else {
      return null;
    }
  }
