...
import parquet.column.statistics.StatisticsOpts;
import parquet.column.statistics.bloomfilter.BloomFilterOptBuilder;
...
public class TestParquetFileWriter {
  ...
  public void testBloomFilterConverter() {
    ...
    IntStatistics convertedBackStats =
        (IntStatistics) parquet.format.converter.ParquetMetadataConverter
            .fromParquetStatistics(thriftStats, PrimitiveTypeName.INT32);
    ...
    thriftStats =
        parquet.format.converter.ParquetMetadataConverter.toParquetStatistics(parquetMRstats);
    convertedBackStats = (IntStatistics) parquet.format.converter.ParquetMetadataConverter
        .fromParquetStatistics(thriftStats, PrimitiveTypeName.INT32);
    assertTrue(parquetMRstats.test(v1));
    assertTrue(convertedBackStats.test(v1));
